...
# %%
# Define helper functions that will be reused throughout the not ebook
...
retry_config = types.HttpRetryOptions(
    attempts=5,  # Maximum retry attemps
...
root_agent = Agent(
    ...
    name="text_chat_bot",
    description="A text chatbot",  # Desciption of the agent's purpose
)
...
# %%
# Run this cell after restarting the kernal. All this history will be gone...
await run_session(
    runner,
...
# Step 1: Create the smae agent (notice we use LlmAgent this time)
...
print("Upgraded to persistent session!")
print(f"  - Database: my_agent_data.db")
print(f"  - Session will survive restarts!")
...
research_app_compacting = App(
    ...
    events_compaction_config=EventsCompactionConfig(
        compaction_interval=3,  # Trigger compaction every 3 in vocations
...
for event in final_session.events:
    # Compaction evetns have a 'compaction' attribute
...
# This demonstrate how tools can write to session state using tool_context.
...
# This is where the distincetion between session-specific and user-specific state becomes important.
